...
namespace embree
{
  ...
  { 
	...
	Vec3fa axis0, axis1;   //!< axis the two strands are aligned into
      ...
      const Split find(const range<size_t>& set, size_t sahBlockSize)
      {
        Vec3fa axis0(0,0,1);
        ...
        const size_t lblocks = (lnum+sahBlockSize-1ull) / sahBlockSize;
        const size_t rblocks = (rnum+sahBlockSize-1ull) / sahBlockSize;
