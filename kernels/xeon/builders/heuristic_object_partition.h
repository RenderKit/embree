...
namespace embree
{
  namespace isa
  {
    ...
    template<>
      inline const ObjectPartition::Split ObjectPartition::find<false>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, BezierRefList& prims, const PrimInfo& pinfo, const size_t logBlockSize)
    {
      BinInfo binner;
      const Mapping mapping(pinfo);
    ...
    template<>
      inline const ObjectPartition::Split ObjectPartition::find<false>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, PrimRefList& prims, const PrimInfo& pinfo, const size_t logBlockSize)
    {
      BinInfo binner;
      const Mapping mapping(pinfo);
      binner.bin(prims,mapping);
    ...
    template<>
      inline const ObjectPartition::Split ObjectPartition::find<false>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, PrimRefList& prims, const PrimInfo& pinfo, const size_t logBlockSize, SplitInfo& sinfo_o)
    {
      BinInfo binner;
      const Mapping mapping(pinfo);
      binner.bin(prims,mapping);
    ...
    template<>
      inline const ObjectPartition::Split ObjectPartition::find<true>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, BezierRefList& prims, const PrimInfo& pinfo, const size_t logBlockSize) {
      return TaskBinParallel<BezierRefList>(threadIndex,threadCount,scheduler,prims,pinfo,logBlockSize).split;
    }

    template<>
      inline const ObjectPartition::Split ObjectPartition::find<true>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, PrimRefList& prims, const PrimInfo& pinfo, const size_t logBlockSize) {
      return TaskBinParallel<PrimRefList>(threadIndex,threadCount,scheduler,prims,pinfo,logBlockSize).split;
    }

    template<>
      inline const ObjectPartition::Split ObjectPartition::find<true>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, PrimRefList& prims, const PrimInfo& pinfo, const size_t logBlockSize, SplitInfo& sinfo_o) 
    {
    ...
    template<>
      inline const std::pair<BBox3fa,BBox3fa> ObjectPartition::computePrimInfoMB<false>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, Scene* scene, BezierRefList& prims)
    {
      BBox3fa bounds0 = empty;
      BBox3fa bounds1 = empty;
    ...
    template<>
      inline const std::pair<BBox3fa,BBox3fa> ObjectPartition::computePrimInfoMB<true>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, Scene* scene, BezierRefList& prims)
    {
    ...
    template<>
    inline void ObjectPartition::Split::split<false>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, 
					      PrimRefBlockAlloc<BezierPrim>& alloc, 
					      BezierRefList& prims, 
					      BezierRefList& lprims_o, PrimInfo& linfo_o, 
					      BezierRefList& rprims_o, PrimInfo& rinfo_o) const
    ...
    template<>
      inline void ObjectPartition::Split::split<true>(size_t threadIndex, size_t threadCount, LockStepTaskScheduler* scheduler, 
