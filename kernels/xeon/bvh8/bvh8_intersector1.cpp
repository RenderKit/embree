...
{ 
  namespace isa
  {
	...
	assert(cur != BVH8::emptyNode);
        ...
        size_t lazy_node = 0;
        PrimitiveIntersector::intersect(pre,ray,0,prim,num,bvh->scene,nullptr,lazy_node);
        ray_far = ray.tfar;
	...
	assert(cur != BVH8::emptyNode);
        ...
        size_t num; Primitive* prim = (Primitive*) cur.leaf(num);
        size_t lazy_node = 0;
        if (PrimitiveIntersector::occluded(pre,ray,0,prim,num,bvh->scene,nullptr,lazy_node)) {
          ray.geomID = 0;
          break;
        }
