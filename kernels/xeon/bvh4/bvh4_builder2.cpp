...
namespace embree
{
  namespace isa
  {
    ...
    __forceinline size_t BVH4Builder2::recurse(size_t threadIndex, size_t threadCount, BVH4Builder2* parent, BuildRecord& record, BuildRecord records_o[BVH4::N])
    {
	...
	*record.dst = parent->createLargeLeaf(threadIndex,record.prims,record.pinfo,record.depth+1); return 0;
	...
	records_o[bestChild].split.split<false>(threadIndex,threadCount,parent->alloc,parent->scene,records_o[bestChild].prims,lrecord.prims,lrecord.pinfo,rrecord.prims,rrecord.pinfo);

	/* fallback if spatial split did fail for corner case */
	if (lrecord.pinfo.size() == 0) {
	  rrecord.split = parent->find<false>(threadIndex,threadCount,record.depth,rrecord.prims,rrecord.pinfo,false);
	...
	if (rrecord.pinfo.size() == 0) {
	  lrecord.split = parent->find<false>(threadIndex,threadCount,record.depth,lrecord.prims,lrecord.pinfo,false);
	...
	//recurse(parent,records_o[i]);
      }
      *record.dst = parent->bvh->encodeNode(node);
      return numChildren;
    }
    
    void BVH4Builder2::BuildTask::recurse(BVH4Builder2* parent, BuildRecord& record)
    {
      /*! compute leaf and split cost */
      const float leafSAH  = parent->primTy.intCost*record.pinfo.leafSAH(parent->logBlockSize);
      const float splitSAH = BVH4::travCost*halfArea(record.pinfo.geomBounds)+parent->primTy.intCost*record.split.splitSAH();
	...
	*record.dst = parent->createLargeLeaf(threadIndex,record.prims,record.pinfo,record.depth+1); return;
	...
	/* count number of replications caused by spatial splits */
	ssize_t remaining = 0;
	const ssize_t replications = lrecord.pinfo.size()+rrecord.pinfo.size()-crecord[bestChild].pinfo.size(); 
	assert(replications >= 0);
	if (replications >= 0) 
	  remaining = atomic_add(&parent->remainingReplications,-replications);
