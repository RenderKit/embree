...
namespace embree
{
  namespace isa 
  {
    ...
    {
    public:
      static void intersect(vbool8* valid, BVH4* bvh, Ray8& ray);
      static void occluded (vbool8* valid, BVH4* bvh, Ray8& ray);
    };
    ...
    public:
      ...
      static __forceinline void intersect1(const BVH4* bvh, NodeRef root, const size_t k, Precalculations& pre, 
					   Ray8& ray, const Vec3vf8 &ray_org, const Vec3vf8 &ray_dir, const Vec3vf8 &ray_rdir, const vfloat8 &ray_tnear, const vfloat8 &ray_tfar, 
					   const Vec3vi8& nearXYZ)
    {
      /*! stack state */
