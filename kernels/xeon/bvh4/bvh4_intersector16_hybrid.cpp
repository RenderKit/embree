...
namespace embree
{
  namespace isa
  {
    ...
    void BVH4Intersector16Hybrid<types,robust,PrimitiveIntersector16>::intersect(vint16* valid_i, BVH4* bvh, Ray16& ray)
    {
...
#pragma unroll(4)
	    for (unsigned i=0; i<BVH4::N; i++)
	    {
	      ...
	      vfloat16 lnearP; const vbool16 lhit = intersect_node<16,robust>(node,i,org,rdir,org_rdir,ray_tnear,ray_tfar,lnearP);
...
#pragma unroll(4)
	    for (unsigned i=0; i<BVH4::N; i++)
	    {
	      ...
	      if (unlikely(child == BVH4::emptyNode)) break;
	      vfloat16 lnearP; const vbool16 lhit = intersect_node<16>(node,i,org,rdir,org_rdir,ray_tnear,ray_tfar,ray.time,lnearP);
    ...
    template<int types, bool robust, typename PrimitiveIntersector16>
    void BVH4Intersector16Hybrid<types,robust,PrimitiveIntersector16>::occluded(vint16* valid_i, BVH4* bvh, Ray16& ray)
    {
      /* verify correct input */
      ...
      while (1) pop:
      {
        ...
        if (unlikely(__popcnt(bits) <= SWITCH_THRESHOLD)) {
          ...
          ray_tfar = select(terminated,vfloat16(neg_inf),ray_tfar);
          continue;
        }
#endif
...
#pragma unroll(4)
    ...
    DEFINE_INTERSECTOR16(BVH4TrianglePairs4Intersector16HybridMoeller, BVH4Intersector16Hybrid<0x1 COMMA true COMMA ArrayIntersectorK_1<16 COMMA TrianglePairsMIntersectorKMoellerTrumbore<4 COMMA 16 COMMA true> > >);
    DEFINE_INTERSECTOR16(BVH4TrianglePairs4Intersector16HybridMoellerNoFilter, BVH4Intersector16Hybrid<0x1 COMMA true COMMA ArrayIntersectorK_1<16 COMMA TrianglePairsMIntersectorKMoellerTrumbore<4 COMMA 16 COMMA false> > >);
