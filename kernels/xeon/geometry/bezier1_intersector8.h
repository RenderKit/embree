...
namespace embree
{
  ...
  struct Bezier1Intersector8
  {
    ...
    static __forceinline void intersect(const Precalculations& pre, Ray8& ray, size_t k, const Bezier1& bezier, const void* geom)
    {
    ...
    retry:
      ...
      if (!likely(geometry->hasIntersectionFilter8())) 
      {
        ...
        ray.Ng.y[k] = T.y;
        ray.Ng.z[k] = T.z;
        ray.geomID[k] = bezier.geomID;
        ray.primID[k] = bezier.primID;
#if defined(__INTERSECTION_FILTER__)
          return;
      }
      ...
      while (true) 
      {
        ...
        Vec3fa P,T; curve3D.eval(uu,P,T);
        if (T != Vec3fa(zero))
          if (runIntersectionFilter8(geometry,ray,k,uu,0.0f,t[i],T,geomID,bezier.primID)) return;
        valid[i] = 0;
    ...
    static __forceinline void intersect(Precalculations& pre, Ray8& ray, const size_t k, const Bezier1* curves, size_t num, void* geom)
    {
      for (size_t i=0; i<num; i++) 
	intersect(pre,ray,k,curves[i],geom);
    }

    static __forceinline void intersect(const avxb& valid_i, Precalculations& pre, Ray8& ray, const Bezier1* curves, size_t num, void* geom)
    {
      int mask = movemask(valid_i);
      while (mask) intersect(pre,ray,__bscf(mask),curves,num,geom);
    }

    static __forceinline bool occluded(const Precalculations& pre, Ray8& ray, const size_t k, const Bezier1& bezier, const void* geom) 
    {
      ...
      while (true) 
      {
        ...
        Vec3fa P,T; curve3D.eval(uu,P,T);
        if (T != Vec3fa(zero))
          if (runOcclusionFilter8(geometry,ray,k,uu,0.0f,t[i],T,geomID,bezier.primID)) break;
        valid[i] = 0;
        if (none(valid)) return false;
        i = select_min(valid,t);
        STAT3(shadow.trav_prim_hits,1,1,1);
    ...
    static __forceinline bool occluded(Precalculations& pre, Ray8& ray, const size_t k, const Bezier1* curves, size_t num, void* geom) 
    {
      for (size_t i=0; i<num; i++) 
    ...
    static __forceinline avxb occluded(const avxb& valid_i, Precalculations& pre, Ray8& ray, const Bezier1* curves, size_t num, void* geom)
    {
      avxb valid_o = false;
      int mask = movemask(valid_i);
      while (mask) {
