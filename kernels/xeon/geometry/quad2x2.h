...
namespace embree
{
  ...
  public:
    ...
    static __forceinline void initFrom3x3Grid( const float* const source, float* const dest,
    ...
    __forceinline void initFrom3x3Grid_discretized( const float4 source[3], unsigned short* const dest)
    {
      ...
      for (size_t i=0;i<4;i++) dest[2+i] = (unsigned short)(row0_b[i]*65535.0f);
      for (size_t i=0;i<4;i++) dest[8+i] = (unsigned short)(row1_b[i]*65535.0f);
      for (size_t i=0;i<4;i++) dest[0+i] = (unsigned short)(row0_a[i]*65535.0f);
      for (size_t i=0;i<4;i++) dest[6+i] = (unsigned short)(row1_a[i]*65535.0f);
    }
