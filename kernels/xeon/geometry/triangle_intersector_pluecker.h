...
namespace embree
{
  namespace isa
  {
    ...
    /*! Test if the ray is occluded by one of the triangles. */
    template<bool enableIntersectionFilter, typename tsimdb, typename tsimdf, typename tsimdi>
      __forceinline bool triangle_occluded_pluecker(const Vec3<tsimdf>& ray_rdir, 
						    const Vec3<tsimdf>& ray_org_rdir, 
						    const Vec3<tsimdf>& ray_dir_scale,
						    Ray& ray, 
						    const Vec3<tsimdf>& tri_v0, 
...
#if ENABLE_NORMALIZED_TRIANGLE_INTERSECTION == 1
          triangle_intersect_pluecker<enableIntersectionFilter,tsimdb,tsimdf,tsimdi>(pre.ray_rdir,pre.ray_org_rdir,pre.ray_dir_scale,ray,tri.v0,tri.v1,tri.v2,tri.geomIDs,tri.primIDs,scene);
#else
...
#if ENABLE_NORMALIZED_TRIANGLE_INTERSECTION == 1
          return triangle_occluded_pluecker<enableIntersectionFilter,tsimdb,tsimdf,tsimdi>(pre.ray_rdir,pre.ray_org_rdir,pre.ray_dir_scale,ray,tri.v0,tri.v1,tri.v2,tri.geomIDs,tri.primIDs,scene);
