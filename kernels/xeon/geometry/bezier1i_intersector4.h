...
namespace embree
{
  ...
  struct Bezier1iIntersector4
  {
    ...
    {
      __forceinline Precalculations (const Ray4& ray, const size_t k)
    ...
    static __forceinline void intersect(const Precalculations& pre, Ray4& ray, const size_t k, const Bezier1i& curve_in, const void* geom)
    {
    ...
    retry:
      ...
      while (true) 
      {
        ...
        Vec3fa P,T; curve3D.eval(uu,P,T);
        if (T != Vec3fa(zero))
          if (runIntersectionFilter4(geometry,ray,k,uu,0.0f,t[i],T,geomID,curve_in.primID)) return;
        valid[i] = 0;
    ...
    static __forceinline void intersect(Precalculations& pre, Ray4& ray, const size_t k, const Bezier1i* curves, size_t num, void* geom)
    {
    ...
    static __forceinline bool occluded(const Precalculations& pre, Ray4& ray, const size_t k, const Bezier1i& curve_in, const void* geom) 
    {
      ...
      while (true) 
      {
        ...
        Vec3fa P,T; curve3D.eval(uu,P,T);
        if (T != Vec3fa(zero))
          if (runOcclusionFilter4(geometry,ray,k,uu,0.0f,t[i],T,geomID,curve_in.primID)) break;
        valid[i] = 0;
        if (none(valid)) return false;
        i = select_min(valid,t);
        STAT3(shadow.trav_prim_hits,1,1,1);
    ...
    static __forceinline bool occluded(Precalculations& pre, Ray4& ray, const size_t k, const Bezier1i* curves, size_t num, void* geom) 
    {
      for (size_t i=0; i<num; i++) 
    ...
    static __forceinline void occluded(const sseb& valid_i, Precalculations& pre, Ray4& ray, const Bezier1i* curves, size_t num, void* geom)
    {
      int mask = movemask(valid_i);
