  ...
  struct Bezier1Intersector4
  {
    ...
    retry:
      ...
      while (true) 
      {
        ...
        Vec3fa P,T; curve3D.eval(uu,P,T);
        if (T != Vec3fa(zero))
          if (runIntersectionFilter4(geometry,ray,k,uu,0.0f,t[i],T,geomID,bezier.primID)) return;
        valid[i] = 0;
    ...
    static __forceinline bool occluded(const Precalculations& pre, Ray4& ray, const size_t k, const Bezier1& bezier, const void* geom) 
    {
      ...
      while (true) 
      {
        ...
        Vec3fa P,T; curve3D.eval(uu,P,T);
        if (T != Vec3fa(zero))
          if (runOcclusionFilter4(geometry,ray,k,uu,0.0f,t[i],T,geomID,bezier.primID)) break;
        valid[i] = 0;
        if (none(valid)) return false;
        i = select_min(valid,t);
        STAT3(shadow.trav_prim_hits,1,1,1);
