    ...
    template<bool list>
      struct Bezier1iIntersector8
      {
        ...
        static __forceinline void intersect(Precalculations& pre, Ray8& ray, const size_t k, const Primitive& curve, void* geom) 
        {
          ...
          BezierIntersector8::intersect(pre,ray,k,a0,a1,a2,a3,curve.geomID<list>(),curve.primID<list>(),geom);
        }
        ...
        static __forceinline bool occluded(Precalculations& pre, Ray8& ray, const size_t k, const Primitive& curve, void* geom) 
        {
          ...
          const Vec3fa a2 = in->vertex(curve.vertexID+2,0);
          const Vec3fa a3 = in->vertex(curve.vertexID+3,0);
          return BezierIntersector8::occluded(pre,ray,k,a0,a1,a2,a3,curve.geomID<list>(),curve.primID<list>(),geom);
        }
