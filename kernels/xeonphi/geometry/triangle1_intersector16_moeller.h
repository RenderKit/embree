...
namespace embree
{
  ...
  template< bool ENABLE_INTERSECTION_FILTER>
	...
	const mic_f _v2 = gather_4f_zlc(and_mask,
					...
					(float*)&tptr[2].v2,
					(float*)&tptr[3].v2);
	const mic_f v0 = _v0 - org_xyz;
	const mic_f v1 = _v1 - org_xyz;
	const mic_f v2 = _v2 - org_xyz;
	...
	const mic_f den     = ldot3_xyz(Ng,dir_xyz);	      
	const mic_f rcp_den = rcp(den);
	...
	const mic_f u = ldot3_xyz(lcross_xyz(v2+v0,e0),dir_xyz) * rcp_den; 
	m_valid       = ge( m_valid, u, zero);

	const mic_f v = ldot3_xyz(lcross_xyz(v0+v1,e1),dir_xyz) * rcp_den; 
	m_valid       = ge( m_valid, v, zero);

	const mic_f w = ldot3_xyz(lcross_xyz(v1+v2,e2),dir_xyz) * rcp_den;  
	m_valid       = ge( m_valid, w, zero);
	...
	for (size_t i=0; i<items; i++,tptr++) 
	  {
	    ...
	    /* load vertices and calculate edges */
	    const mic_f v0 = broadcast4to16f(&tri.v0) - org.x;
	    const mic_f v1 = broadcast4to16f(&tri.v1) - org.y;
	    const mic_f v2 = broadcast4to16f(&tri.v2) - org.z;
...
#if defined(RTCORE_BACKFACE_CULLING)	    
	    mic_m valid = den > zero;
#else
	    mic_m valid = MIC_M_ALL;
#endif
