	...
	if (unlikely(any(m_final)))
	  {
		...
		while (any(m_final)) 
		  {
		    ...
		    if (likely(!geom->hasIntersectionFilter<vfloat16>())) 
		      {
			ray16.updateK(bitscan(m_tri)/*m_tri*/,rayIndex,min_dist,u,v,gnormalx,gnormaly,gnormalz,geomID,vint16(primID));
			break;
		      }
		...
		ray16.updateK(bitscan(m_tri)/*m_tri*/,rayIndex,min_dist,u,v,gnormalx,gnormaly,gnormalz,tri_ptr->geomID(),vint16(tri_ptr->primID()));
	      }
	    return true;
