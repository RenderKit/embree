...
namespace embree
{
  namespace isa
  {
    ...
    void BVH4iIntersector16Hybrid<LeafIntersector,ENABLE_COMPRESSED_BVH4I_NODES,ROBUST>::intersect(vint16* valid_i, BVH4i* bvh, Ray16& ray16)
    {
      /* near and node stack */
...
#pragma unroll(4)
		...
		lclipMinX = msub(vfloat16(clower[4*i+0]),rdir16.x,org_rdir16.x);
		lclipMinY = msub(vfloat16(clower[4*i+1]),rdir16.y,org_rdir16.y);
		lclipMinZ = msub(vfloat16(clower[4*i+2]),rdir16.z,org_rdir16.z);
		lclipMaxX = msub(vfloat16(cupper[4*i+0]),rdir16.x,org_rdir16.x);
		lclipMaxY = msub(vfloat16(cupper[4*i+1]),rdir16.y,org_rdir16.y);
		lclipMaxZ = msub(vfloat16(cupper[4*i+2]),rdir16.z,org_rdir16.z);		
	      }
    ...
    template<typename LeafIntersector, bool ENABLE_COMPRESSED_BVH4I_NODES, bool ROBUST>
    void BVH4iIntersector16Hybrid<LeafIntersector,ENABLE_COMPRESSED_BVH4I_NODES,ROBUST>::occluded(vint16* valid_i, BVH4i* bvh, Ray16& ray16)
    {
      /* allocate stack */
      __aligned(64) vfloat16   stack_dist[3*BVH4i::maxDepth+1];
      __aligned(64) NodeRef stack_node[3*BVH4i::maxDepth+1];
	...
	const unsigned int leaf_mask = BVH4I_LEAF_MASK;
	  ...
	  vbool16 m_curUtil = gt(ray_tfar,curDist);
