...
namespace embree
{
  namespace isa
  {
    ...
    template<int N,int Nx>
      __forceinline size_t intersectNodeRobust(const typename BVHN<N>::AlignedNode* node, const TravRay<N,Nx>& ray, const vfloat<Nx>& tnear, const vfloat<Nx>& tfar, vfloat<Nx>& dist) 
    {      
    ...
    template<>
      __forceinline size_t intersectNodeRobust<4,16>(const typename BVHN<4>::AlignedNode* node, const TravRay<4,16>& ray, const vfloat<16>& tnear, const vfloat<16>& tfar, vfloat<16>& dist) 
    {      
    ...
    template<>
      __forceinline size_t intersectNodeRobust<8,16>(const typename BVHN<8>::AlignedNode* node, const TravRay<8,16>& ray, const vfloat<16>& tnear, const vfloat<16>& tfar, vfloat<16>& dist) 
    {      
    ...
    template<int N, int Nx>
      __forceinline size_t intersectNode(const typename BVHN<N>::AlignedNode* node, const TravRay<N,Nx>& ray, const vfloat<Nx>& tnear, const vfloat<Nx>& tfar, vfloat<Nx>& dist);

    template<>
      __forceinline size_t intersectNode<4,4>(const typename BVH4::AlignedNode* node, const TravRay<4,4>& ray, const vfloat4& tnear, const vfloat4& tfar, vfloat4& dist)
    {
#if defined (__AVX2__)
    ...
    template<>
      __forceinline size_t intersectNode<8,8>(const typename BVH8::AlignedNode* node, const TravRay<8,8>& ray, const vfloat8& tnear, const vfloat8& tfar, vfloat8& dist)
    {
#if defined (__AVX2__)
      ...
      __forceinline size_t intersectNode<4,4>(const typename BVH4::QuantizedNode* node, const TravRay<4,4>& ray, const vfloat4& tnear, const vfloat4& tfar, vfloat4& dist)
    {
      ...
      const vfloat4 lower_x = node->dequantize<4>(ray.nearX >> 2) * scale_x + start_x;
      const vfloat4 upper_x = node->dequantize<4>(ray.farX  >> 2) * scale_x + start_x;
      const vfloat4 start_y(node->start.y);
      const vfloat4 scale_y(node->scale.y);
      const vfloat4 lower_y = node->dequantize<4>(ray.nearY >> 2) * scale_y + start_y;
      const vfloat4 upper_y = node->dequantize<4>(ray.farY  >> 2) * scale_y + start_y;
      ...
      __forceinline size_t intersectNode<8,8>(const typename BVH8::QuantizedNode* node, const TravRay<8,8>& ray, const vfloat8& tnear, const vfloat8& tfar, vfloat8& dist)
    {
      ...
      const vfloat8 lower_x = node->dequantize<8>(ray.nearX >> 2) * scale_x + start_x;
      const vfloat8 upper_x = node->dequantize<8>(ray.farX  >> 2) * scale_x + start_x;
      const vfloat8 start_y(node->start.y);
      const vfloat8 scale_y(node->scale.y);
      const vfloat8 lower_y = node->dequantize<8>(ray.nearY >> 2) * scale_y + start_y;
      const vfloat8 upper_y = node->dequantize<8>(ray.farY  >> 2) * scale_y + start_y;
      const vfloat8 start_z(node->start.z);
      const vfloat8 scale_z(node->scale.z);
      const vfloat8 lower_z = node->dequantize<8>(ray.nearZ >> 2) * scale_z + start_z;
      const vfloat8 upper_z = node->dequantize<8>(ray.farZ  >> 2) * scale_z + start_z;
      ...
      __forceinline size_t intersectNode<4,16>(const typename BVH4::QuantizedNode* node, const TravRay<4,16>& ray, const vfloat16& tnear, const vfloat16& tfar, vfloat16& dist)
    {
      ...
      const vfloat16 lower_x = vfloat16(node->dequantize<16>(ray.nearX >> 2)) * scale_x + start_x;
      const vfloat16 upper_x = vfloat16(node->dequantize<16>(ray.farX  >> 2)) * scale_x + start_x;
      const vfloat16 start_y(node->start.y);
      const vfloat16 scale_y(node->scale.y);
      const vfloat16 lower_y = vfloat16(node->dequantize<16>(ray.nearY >> 2)) * scale_y + start_y;
      const vfloat16 upper_y = vfloat16(node->dequantize<16>(ray.farY  >> 2)) * scale_y + start_y;
      const vfloat16 start_z(node->start.z);
      const vfloat16 scale_z(node->scale.z);
      const vfloat16 lower_z = vfloat16(node->dequantize<16>(ray.nearZ >> 2)) * scale_z + start_z;
      const vfloat16 upper_z = vfloat16(node->dequantize<16>(ray.farZ  >> 2)) * scale_z + start_z;
