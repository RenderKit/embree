...
namespace embree
{
    ...
    struct __aligned(8) QuantizedBaseNode_t
  {
                                       ...
                                       float &scale)
    {
      ...
      vint<N> ilower = max(toInt(floor((lower - vfloat<N>(minF))*vfloat<N>(encode_scale))),MIN_QUAN);
      vint<N> iupper = min(toInt(ceil ((upper - vfloat<N>(minF))*vfloat<N>(encode_scale))),MAX_QUAN);
