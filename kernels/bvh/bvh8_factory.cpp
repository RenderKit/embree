...
namespace embree
{
  ...
  Accel* BVH8Factory::BVH8Triangle4iMB4D(Scene* scene, BuildVariant bvariant, IntersectVariant ivariant)
  {
    BVH8* accel = new BVH8(Triangle4iMB::type,scene);
    ...
    /*if (scene->device->tri_builder_mb == "default") { // FIXME: implement
      switch (bvariant) {
      case BuildVariant::STATIC      : builder = BVH8Triangle4iMB4DSceneBuilderSAH(accel,scene,0); break;
      case BuildVariant::DYNAMIC     : assert(false); break; // FIXME: implement
      case BuildVariant::HIGH_QUALITY: assert(false); break;
      }
    }
    else if (scene->device->tri_builder_mb == "sah")  builder = BVH8Triangle4iMB4DSceneBuilderSAH(accel,scene,0);
