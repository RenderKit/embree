...
namespace embree
{
  ...
  DECLARE_ISA_FUNCTION(Builder*,BVH8InstanceExpensiveSceneBuilderSAH,void* COMMA Scene* COMMA size_t);
  DECLARE_ISA_FUNCTION(Builder*,BVH8InstanceExpensiveMBSceneBuilderSAH,void* COMMA Scene* COMMA size_t);
  ...
  void BVH8Factory::selectBuilders(int features)
  {
    ...
    IF_ENABLED_INSTANCE(SELECT_SYMBOL_INIT_AVX(features,BVH8InstanceExpensiveSceneBuilderSAH));
    IF_ENABLED_INSTANCE(SELECT_SYMBOL_INIT_AVX(features,BVH8InstanceExpensiveMBSceneBuilderSAH));
...
#define ENABLE_WOOP_TEST 0
  ...
  Accel* BVH8Factory::BVH8InstanceExpensive(Scene* scene, BuildVariant bvariant)
  {
    BVH8* accel = new BVH8(InstancePrimitive::type,scene);
    Accel::Intersectors intersectors = BVH8InstanceIntersectors(accel);
    Builder* builder = BVH8InstanceExpensiveSceneBuilderSAH(accel,scene,0);
    return new AccelInstance(accel,builder,intersectors);
  }

  Accel* BVH8Factory::BVH8InstanceExpensiveMB(Scene* scene)
  {
    BVH8* accel = new BVH8(InstancePrimitive::type,scene);
    Accel::Intersectors intersectors = BVH8InstanceMBIntersectors(accel);
    Builder* builder = BVH8InstanceExpensiveMBSceneBuilderSAH(accel,scene,0);
    return new AccelInstance(accel,builder,intersectors);
  }
