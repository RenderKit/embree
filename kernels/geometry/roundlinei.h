...
namespace embree
{
  ...
  public:
    ...
    static __forceinline size_t bytes(size_t N) { return blocks(N)*sizeof(RoundLineMi); }

  public:

    /* Default constructor */
    __forceinline RoundLineMi() {  }

    /* Construction from vertices and IDs */
    __forceinline RoundLineMi	(const vuint<M>& v0, 
                               const vuint<M>& geomIDs, const vuint<M>& primIDs, Geometry::GType gtype)
    ...
    __forceinline void fill(const PrimRefT* prims, size_t& begin, size_t end, Scene* scene)
    {
      ...
      new (this) RoundLineMi(v0,geomID,primID,gty); // FIXME: use non temporal store
    }
      ...
      __forceinline static typename BVH::NodeRef createLeaf (BVH* bvh, const PrimRef* prims, const range<size_t>& set, const Allocator& alloc)
    {
      ...
      size_t items = RoundLineMi::blocks(set.size());
      size_t numbytes = RoundLineMi::bytes(set.size());
      RoundLineMi* accel = (RoundLineMi*) alloc.malloc1(numbytes,M*sizeof(float));
    ...
    __forceinline static typename BVH::NodeRecordMB4D createLeafMB(BVH* bvh, const SetMB& prims, const Allocator& alloc)
    {
      ...
      size_t items = RoundLineMi::blocks(prims.size());
      size_t numbytes = RoundLineMi::bytes(prims.size());
      RoundLineMi* accel = (RoundLineMi*) alloc.malloc1(numbytes,M*sizeof(float));
    ...
    /*! output operator */
    friend __forceinline std::ostream& operator<<(std::ostream& cout, const RoundLineMi& line) {
      return cout << "RoundLine" << M << "i {" << line.v0 << ", " << line.vL << ", " << line.vR << ", " << line.geomID() << ", " << line.primID() << "}";
    }
  ...
  template<>
    __forceinline void RoundLineMi<4>::gather(Vec4vf4& p0,
                                              Vec4vf4& p1,
                                              Vec4vf4& pL,
                                              Vec4vf4& pR,
                                              const Scene* scene) const
  ...
  template<>
    __forceinline void RoundLineMi<8>::gather(Vec4vf8& p0,
                                              Vec4vf8& p1,
                                              Vec4vf8& pL,
                                              Vec4vf8& pR,
                                              const Scene* scene) const
  ...
  template<int M>
    typename RoundLineMi<M>::Type RoundLineMi<M>::type;
  
  typedef RoundLineMi<4> RoundLine4i;
  typedef RoundLineMi<8> RoundLine8i;
}
