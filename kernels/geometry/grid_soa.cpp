        ...
        gridOffset(unsigned(gridOffset)), gridBytes(unsigned(gridBytes)), rootOffset(unsigned(gridOffset+time_steps*gridBytes))
    {
      ...
      for (size_t t=0; t<time_steps; t++)
      {
        ...
        for (unsigned i=0; i<dim_offset; i+=VSIZEX) {
          const vintx iu = toInt(clamp(vfloatx::load(&local_grid_u[i])*(0x10000/8.0f), vfloatx(0.0f), vfloatx(0xFFFF)));
          const vintx iv = toInt(clamp(vfloatx::load(&local_grid_v[i])*(0x10000/8.0f), vfloatx(0.0f), vfloatx(0xFFFF)));
