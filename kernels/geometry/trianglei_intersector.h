    ...
    struct TriangleMiIntersectorKMoeller
    {
      ...
      static __forceinline void intersect(const vbool<K>& valid_i, Precalculations& pre, RayHitK<K>& ray, IntersectContext* context, const Primitive& tri)
      {
        ...
        for (size_t i=0; i<Primitive::max_size(); i++)
        {
          ...
          const Vec3vf<K> v0 = tri.getVertex(tri.getVertexID(Primitive::VertexIDs::V0,scene),i,scene);
          const Vec3vf<K> v1 = tri.getVertex(tri.getVertexID(Primitive::VertexIDs::V1,scene),i,scene);
          const Vec3vf<K> v2 = tri.getVertex(tri.getVertexID(Primitive::VertexIDs::V2,scene),i,scene);
