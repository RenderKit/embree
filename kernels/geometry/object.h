...
namespace embree
{
  ...
  public:
    ...
    /*! fill triangle from triangle list */
    __forceinline std::pair<BBox3fa,BBox3fa> fill_mblur(const PrimRef* prims, size_t& i, size_t end, Scene* scene, const bool list, size_t time)
    {
      const PrimRef& prim = prims[i]; i++;
      ...
      return accel->bounds_mblur(primID,time);
    }
