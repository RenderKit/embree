    ...
    struct QuadMiIntersectorKMoeller
    {
      ...
      static __forceinline void intersect(const vbool<K>& valid_i, Precalculations& pre, RayHitK<K>& ray, IntersectContext* context, const QuadMi<M>& quad)
      {
        ...
        for (size_t i=0; i<QuadMi<M>::max_size(); i++)
        {
          ...
          const Vec3vf<K> p0 = quad.getVertex(quad.getVertexID(Primitive::VertexIDs::V0,scene),i,scene);
          const Vec3vf<K> p1 = quad.getVertex(quad.getVertexID(Primitive::VertexIDs::V1,scene),i,scene);
          const Vec3vf<K> p2 = quad.getVertex(quad.getVertexID(Primitive::VertexIDs::V2,scene),i,scene);
          const Vec3vf<K> p3 = quad.getVertex(quad.getVertexID(Primitive::VertexIDs::V3,scene),i,scene);
