...
namespace embree
{
  ...
  template<typename QInternalNode>
  void QBVH6::calculateInternalNodeRelocationData(QBVH6::Node node, uint32_t myid,
                      std::map<QBVH6*, uint32_t>& id, std::map<uint64_t, QBVH6*>& link2bvh, 
                      std::vector<std::vector<uint64_t>>& relocationData)
  {
    QInternalNode* inner = node.innerNode<QInternalNode>();
    for (uint32_t i = 0; i < QInternalNode::NUM_CHILDREN; i++)
    {
      if (inner->valid(i))
        calculateRelocationData(inner->child(i), myid, id, link2bvh, relocationData);
    }
  }

  void QBVH6::calculateRelocationData(QBVH6::Node node, uint32_t myid,
                      std::map<QBVH6*,uint32_t>& id, std::map<uint64_t,QBVH6*>& link2bvh, 
                      std::vector<std::vector<uint64_t>>& relocationData)
  {
    switch (node.type)
    ...
    case NODE_TYPE_INTERNAL:
    {
      calculateInternalNodeRelocationData<QBVH6::InternalNode6>(node, myid, id, link2bvh, relocationData);
      break;
    }
    case NODE_TYPE_PROCEDURAL: {
      PrimLeafDesc* leaf = (PrimLeafDesc*)node.node;
      if (leaf->opaqueCullingEnabled()) break;
    }
      
    case NODE_TYPE_INSTANCE:
    {
      InstanceLeaf* inst = (InstanceLeaf*)node.node;
      link2bvh[inst->part0.startNodePtr] = (QBVH6*)(std::ptrdiff_t) inst->part1.bvhPtr;
      link2bvh[inst->part1.bvhPtr] = (QBVH6*)(std::ptrdiff_t) inst->part1.bvhPtr;
      relocationData[myid].push_back(inst->startNodePtrAddr() - uint64_t(this));
      relocationData[myid].push_back(inst->bvhPtrAddr() - uint64_t(this));
      ((QBVH6*)(std::ptrdiff_t)inst->part1.bvhPtr)->calculateRelocationData(id,link2bvh,relocationData);
      break;
    }
    case NODE_TYPE_QUAD: break;
    default: assert(false); break;
    }
  }

  void QBVH6::calculateRelocationData(std::map<QBVH6*,uint32_t>& id,std::map<uint64_t,QBVH6*>& link2bvh,std::vector<std::vector<uint64_t>>& relocationData)
  {
    if (id.find(this) != id.end()) return;

    /* allocate new ID  */
    uint32_t myid = (uint32_t)relocationData.size();
    id[this] = myid;
    relocationData.push_back(std::vector<uint64_t>());
    calculateRelocationData(root(),myid,id,link2bvh,relocationData);
  }
}
