...
namespace embree
{
  ...
  struct Ray4
  {
    ...
    __forceinline bool4 valid() const {
      const bool4 vx = abs(org.x) < float4(inf) & abs(dir.x) < float4(inf);
      const bool4 vy = abs(org.y) < float4(inf) & abs(dir.y) < float4(inf);
      const bool4 vz = abs(org.z) < float4(inf) & abs(dir.z) < float4(inf);
