    ...
    __forceinline AffineSpace3vf<K> getWorld2LocalSlerp(size_t i, const vbool<K>& valid, const vfloat<K>& t) const
    {
      ...
      if (likely(all(valid, itime_k == vint<K>(itime)))) {
        ...
        while (any(valid1)) {
          ...
          space0 = select(valid2, AffineSpace3vff<K>(l2w(i, itime2+0)), space0);
          space1 = select(valid2, AffineSpace3vff<K>(l2w(i, itime2+1)), space1);
        }
    ...
    __forceinline AffineSpace3vf<K> getWorld2LocalLerp(size_t i, const vbool<K>& valid, const vfloat<K>& t) const
    {
      ...
      if (likely(all(valid, itime_k == vint<K>(itime)))) {
        ...
        while (any(valid1)) {
          vbool<K> valid2;
          const int itime2 = next_unique(valid1, itime_k, valid2);
          space0 = select(valid2, AffineSpace3vf<K>((AffineSpace3fa)l2w(i, itime2+0)), space0);
          space1 = select(valid2, AffineSpace3vf<K>((AffineSpace3fa)l2w(i, itime2+1)), space1);
        }
