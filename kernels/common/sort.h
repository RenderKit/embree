...
namespace embree
{
  ...
  //{
    ...
    public:
      ...
      ParallelSortUInt32 ()
      {
        const size_t numThreads = getNumberOfLogicalThreads(); // FIMXE: what happens if we get called with too many threads?
	...
	class ParallelTask
      {
      public:
	ParallelTask (ParallelSortUInt32* parent, Ty* src, Ty* tmp, Ty* dst, const size_t N)
	...
	{
	  ThreadRadixCountTy* radixCount = parent->radixCount;
	...
	static void task_radixsort (void* data, const size_t threadIndex, const size_t threadCount) { 
      ...
      private:
	ParallelSortUInt32* const parent;
	Ty* const src;
	Ty* const tmp;
	Ty* const dst;
	const size_t N;
