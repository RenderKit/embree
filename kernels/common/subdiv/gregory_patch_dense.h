...
{  
  ...
  public:
    ...
    {
#if defined(__MIC__)
      const vfloat16 row0 = load16f(&matrix[0][0]);
      const vfloat16 row1 = load16f(&matrix[1][0]);
      const vfloat16 row2 = load16f(&matrix[2][0]);
      const vfloat16 row3 = load16f(&matrix[3][0]);
