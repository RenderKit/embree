    ...
    template<typename vbool, typename vint, typename vfloat, typename Vertex, typename Vertex_t = Vertex>
      ...
      public:
        ...
        : P(P), dPdu(dPdu), dPdv(dPdv), dstride(dstride), N(N)
        {
          ...
          const vbool valid1 = patch ? eval(valid0,patch,u,v,1.0f) : vbool(false);
        ...
        vbool eval_quad(const vbool& valid, const typename Patch::SubdividedQuadPatch* This, const vfloat& u, const vfloat& v, const float dscale)
        {
          vbool ret = false;
          ...
          if (any(u0v0_mask)) ret |= eval(u0v0_mask,This->child[0],2.0f*u,2.0f*v,2.0f*dscale);
          if (any(u1v0_mask)) ret |= eval(u1v0_mask,This->child[1],2.0f*u-1.0f,2.0f*v,2.0f*dscale);
          if (any(u1v1_mask)) ret |= eval(u1v1_mask,This->child[2],2.0f*u-1.0f,2.0f*v-1.0f,2.0f*dscale);
          if (any(u0v1_mask)) ret |= eval(u0v1_mask,This->child[3],2.0f*u,2.0f*v-1.0f,2.0f*dscale);
          return ret;
        }
        
        vbool eval_general_triangle(const vbool& valid, const typename Patch::SubdividedGeneralTrianglePatch* This, const vfloat& u, const vfloat& v)
        {
          vbool ret = false;
          ...
          if  (any(tri0_mask)) {
            ...
            ret |= eval(tri0_mask,This->child[0],xy.x,xy.y,1.0f);
          ...
          if (any(tri1_mask)) {
            ...
            ret |= eval(tri1_mask,This->child[1],xy.x,xy.y,1.0f);
          ...
          if (any(tri2_mask)) {
            ...
            ret |= eval(tri2_mask,This->child[2],xy.x,xy.y,1.0f);
        ...
        vbool eval(const vbool& valid, Ref This, const vfloat& u, const vfloat& v, const float dscale) 
        {
          ...
          case Patch::SUBDIVIDED_QUAD_PATCH: {
            return eval_quad(valid,((typename Patch::SubdividedQuadPatch*)This.object()),u,v,dscale);
          }
          case Patch::SUBDIVIDED_GENERAL_TRIANGLE_PATCH: { 
            assert(dscale == 1.0f); 
            return eval_general_triangle(valid,((typename Patch::SubdividedGeneralTrianglePatch*)This.object()),u,v); 
          }
          case Patch::SUBDIVIDED_GENERAL_PATCH: { 
            assert(dscale == 1.0f); 
            return eval_general(valid,((typename Patch::SubdividedGeneralPatch*)This.object()),u,v); 
          }
          default: 
            assert(false); 
            return false;
