...
namespace embree
{
  Points::Points (Scene* parent, RTCGeometryFlags flags, size_t numPrimitives, size_t numTimeSteps)
    : Geometry(parent,POINTS,numPrimitives,numTimeSteps,flags)
  {
    for (size_t i=0; i<numTimeSteps; i++) {
		vertices[i].init(parent->device, numPrimitives, sizeof(Vec3fa));
    }
    enabling();
  }
  ...
  {
    if (numTimeSteps == 1) atomic_add(&parent->world1.numPoints,numPrimitives);
    else                   atomic_add(&parent->world2.numPoints,numPrimitives);
  }

  void Points::disabling()
  {
    if (numTimeSteps == 1) atomic_add(&parent->world1.numPoints,-(ssize_t)numPrimitives);
    else                   atomic_add(&parent->world2.numPoints,-(ssize_t)numPrimitives);
  }
  ...
  void Points::setBuffer(RTCBufferType type, void* ptr, size_t offset, size_t stride)
  {
    if (parent->isStatic() && parent->isBuild())
      throw_RTCError(RTC_INVALID_OPERATION,"static geometries cannot get modified");
    ...
    case RTC_INDEX_BUFFER  :
      throw_RTCError(RTC_INVALID_OPERATION,"points do not accept index buffers");
      break;
  ...
  void* Points::map(RTCBufferType type)
  {
    if (parent->isStatic() && parent->isBuild()) {
      throw_RTCError(RTC_INVALID_OPERATION,"static geometries cannot get modified");
      return nullptr;
    ...
    switch (type) {
    case RTC_INDEX_BUFFER  : throw_RTCError(RTC_INVALID_OPERATION,"points do not accept index buffers"); break;
  ...
  void Points::unmap(RTCBufferType type)
  {
    if (parent->isStatic() && parent->isBuild())
      throw_RTCError(RTC_INVALID_OPERATION,"static geometries cannot get modified");
  ...
  void Points::immutable ()
  {
    const bool freeVertices = !parent->needPointVertices;
  ...
  void Points::interpolate(unsigned primID, float u, float v, RTCBufferType buffer, float* P, float* dPdu, float* dPdv, size_t numFloats)
  {
  ...
  void Points::write(std::ofstream& file)
  {
    int type = POINTS;
