...
namespace embree
{
  GeometryInstance::GeometryInstance (Device* device, Ref<Geometry> geom)
    : Geometry(device,geom->getType(), 1, geom->numTimeSteps), local2world(one), world2local(one), geom(geom) {}
  ...
  void GeometryInstance::count(const Ref<Geometry>& geom, ssize_t f)
  {
    ...
    {
      switch (geom->getType()) {
      case GTY_TRIANGLE_MESH: scene->instanced.numTriangles      += f*ssize_t(geom->size()); break;
    ...
    {
      switch (geom->getType()) {
      case GTY_TRIANGLE_MESH: scene->instancedMB.numTriangles      += f*ssize_t(geom->size()); break;
  ...
  {
    geom->used++;
    if (geom->getType() == Geometry::GTY_GROUP) {
  ...
  GeometryGroup::GeometryGroup (Device* device, const std::vector<Ref<Geometry>>& geometries) 
    : Geometry(device,Geometry::GTY_GROUP, (unsigned int)geometries.size(), 1), geometries(geometries)
  {
  }
