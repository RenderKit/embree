...
namespace embree
{
  ...
  RTCAlgorithmFlags aflags = (RTCAlgorithmFlags) (RTC_INTERSECT1 | RTC_INTERSECT4 | RTC_INTERSECT8 | RTC_INTERSECT16 | RTC_INTERSECT_STREAM);
...
#define STREAM_SIZE 256
      ...
      for (size_t i=0; i<N; i+=STREAM_SIZE) {
        ...
        if (intersect)
          rtcIntersect1M(scene,rays,STREAM_SIZE,sizeof(RTCRay),RTC_RAYN_INCOHERENT);
        else
          rtcOccluded1M(scene,rays,STREAM_SIZE,sizeof(RTCRay),RTC_RAYN_INCOHERENT);
      }        
    ...
    static double benchmark_rtcore_intersect_coherent_stream_throughput_thread(void* arg) 
    {
      ...
      for (size_t yy=0; yy<height; yy+=COHERENT_STREAM_TILE_Y) {
        for (size_t xx=0; xx<width; xx+=COHERENT_STREAM_TILE_X) {
          ...
          rtcIntersect1M(scene,rays,index,sizeof(RTCRay),RTC_RAYN_INCOHERENT);
        }
      }
