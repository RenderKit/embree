...
unsigned int addQuadCube (RTCScene scene, const Vec3fa& pos, unsigned int num_time_steps)
{
  ...
  RTCGeometry geom = rtcNewQuadMesh (g_device, RTC_BUILD_QUALITY_MEDIUM);
...
{
  RTCGeometry geom = rtcNewCurveGeometry(g_device, RTC_BUILD_QUALITY_MEDIUM, type, RTC_BASIS_BSPLINE);
...
{
  RTCGeometry geom = rtcNewCurveGeometry (g_device, RTC_BUILD_QUALITY_MEDIUM, RTC_CURVE_RIBBON, RTC_BASIS_LINEAR);
...
{
  RTCGeometry geom = rtcNewUserGeometry(g_device,RTC_BUILD_QUALITY_MEDIUM,1,num_time_steps);
