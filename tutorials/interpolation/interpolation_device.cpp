...
{
  RTCGeometry geom = rtcNewSubdivisionMesh(g_device, RTC_GEOMETRY_DYNAMIC, 1);
...
{
  RTCGeometry geom = rtcNewTriangleMesh(g_device, RTC_GEOMETRY_DYNAMIC, 1);
...
{
  RTCGeometry geom = rtcNewQuadMesh(g_device, RTC_GEOMETRY_DYNAMIC, 1);
...
{
  RTCGeometry geom = rtcNewCurveGeometry (g_device, RTC_GEOMETRY_DYNAMIC, RTC_CURVE_SURFACE, RTC_BASIS_BEZIER, 1);
  ...
  if (ray.geomID != RTC_INVALID_GEOMETRY_ID)
  {
    ...
    if (ray.geomID > 0)
    {
      unsigned int geomID = ray.geomID; {
        rtcInterpolate(rtcGetGeometry(g_scene,geomID),ray.primID,ray.u,ray.v,RTC_USER_VERTEX_BUFFER0,&diffuse.x,nullptr,nullptr,3);
      }
      //return diffuse;
    ...
    if (ray.geomID >= 3) {
      Vec3fa dPdu,dPdv;
      unsigned int geomID = ray.geomID; {
        rtcInterpolate(rtcGetGeometry(g_scene,geomID),ray.primID,ray.u,ray.v,RTC_VERTEX_BUFFER0,nullptr,&dPdu.x,&dPdv.x,3);
      }
      //return dPdu;
      Ng = cross(dPdv,dPdu);
    }
