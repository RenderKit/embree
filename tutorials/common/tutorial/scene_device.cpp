...
namespace embree
{
  ...
  unsigned int ConvertSubdivMesh(ISPCSubdivMesh* mesh, RTCGeometryFlags gflags, RTCScene scene_out)
  {
    ...
    if (mesh->normals) {
      rtcSetBuffer(scene_out, geomID, RTC_USER_VERTEX_BUFFER_(1), mesh->normals, 0, sizeof(Vec3fa  ), mesh->numNormals);
      if (mesh->normal_indices) {
        rtcSetBuffer(scene_out, geomID, RTC_INDEX_BUFFER_(1),  mesh->normal_indices  , 0, sizeof(unsigned int), mesh->numEdges);
        rtcSetIndexBuffer(scene_out, geomID, RTC_USER_VERTEX_BUFFER_(1), RTC_INDEX_BUFFER_(1));
    ...
    if (mesh->texcoords) {
      rtcSetBuffer(scene_out, geomID, RTC_USER_VERTEX_BUFFER_(2), mesh->texcoords, 0, sizeof(Vec2f), mesh->numTexCoords);
      if (mesh->texcoord_indices) {
        rtcSetBuffer(scene_out, geomID, RTC_INDEX_BUFFER_(2),  mesh->texcoord_indices  , 0, sizeof(unsigned int), mesh->numEdges);
        rtcSetIndexBuffer(scene_out, geomID, RTC_USER_VERTEX_BUFFER_(2), RTC_INDEX_BUFFER_(2));
