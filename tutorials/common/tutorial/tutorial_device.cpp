...
Vec3fa renderPixelTexCoords(float x, float y, const ISPCCamera& camera, RayStats& stats)
{
  ...
  else if (g_ispc_scene)
  {
    ...
    unsigned int geomID = ray.geomID; {
      ...
      rtcInterpolate0(geometry,ray.primID,ray.u,ray.v,RTC_USER_VERTEX_BUFFER_(2),&st.x,2);
    }
...
Vec3fa renderPixelDifferentials(float x, float y, const ISPCCamera& camera, RayStats& stats)
{
  ...
  unsigned int geomID = ray.geomID; {
    RTCGeometry geometry = rtcGetGeometry(g_scene,geomID);
    rtcInterpolate1(geometry,ray.primID,ray.u+0.f,ray.v+0.f,RTC_VERTEX_BUFFER0,&P00.x,&dP00du.x,&dP00dv.x,3);
    rtcInterpolate1(geometry,ray.primID,ray.u+0.f,ray.v+eps,RTC_VERTEX_BUFFER0,&P01.x,&dP01du.x,&dP01dv.x,3);
    rtcInterpolate1(geometry,ray.primID,ray.u+eps,ray.v+0.f,RTC_VERTEX_BUFFER0,&P10.x,&dP10du.x,&dP10dv.x,3);
    rtcInterpolate1(geometry,ray.primID,ray.u+eps,ray.v+eps,RTC_VERTEX_BUFFER0,&P11.x,&dP11du.x,&dP11dv.x,3);
