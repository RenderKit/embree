...
/* Device properties that can be queried using the rtcGetDeviceParameter function. */
enum RTCDeviceProperty {
  ...
  RTC_DEVICE_PROPERTY_NATIVE_RAY4_SUPPORTED = 2,           // flag whether native ray packets of size 4 are supported
  RTC_DEVICE_PROPERTY_NATIVE_RAY8_SUPPORTED = 3,           // flag whether native ray packets of size 8 are supported
  RTC_DEVICE_PROPERTY_NATIVE_RAY16_SUPPORTED = 4,          // flag whether native ray packets of size 16 are supported
  ...
  RTC_DEVICE_PROPERTY_JOIN_COMMIT_SUPPORTED = 30,          // flag whether rtcJoinCommitScene can be used to join build operation (not supported when compiled with some older TBB versions) (read only)
};
...
/* Error codes returned by the rtcGetDeviceError function. */
enum RTCError {
...
/* Sets the memory consumption callback function which is
