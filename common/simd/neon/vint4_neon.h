...
namespace embree
{
  ...
  struct vint<4>
  {
    ...
    __forceinline vint4 &operator=(const vint4 &a)
    {
      v = a.v;
      return *this;
    }
    ...
    static __forceinline vint4 gather(const vboolf4 &mask, const int *ptr, const vint4 &index)
    {
      vint4 r = zero;
      if (likely(mask[0]))
        r[0] = *(int *)(((char *)ptr) + scale * index[0]);
      if (likely(mask[1]))
        r[1] = *(int *)(((char *)ptr) + scale * index[1]);
      if (likely(mask[2]))
        r[2] = *(int *)(((char *)ptr) + scale * index[2]);
      if (likely(mask[3]))
        r[3] = *(int *)(((char *)ptr) + scale * index[3]);
      return r;
    }
  ...
  template <int src>
  __forceinline int extract(const vint4 &b) { return b[src & 3]; }
  template <>
  __forceinline int extract<0>(const vint4 &b) { return vgetq_lane_s32(b.v, 0); }
  template <int dst>
  __forceinline vint4 insert(const vint4 &a, int b)
  {
    vint4 c = a;
    c[dst & 3] = b;
    return c;
