...
namespace embree
{
  ...
  struct vllong<4>
  {
    ...
    __forceinline vllong4 &operator=(const vllong4 &f)
    {
      v = f.v;
      return *this;
    }

    __forceinline vllong(const int32x4_t &a_lo, const int32x4_t &a_hi)
    {
      v.lo = a_lo;
      v.hi = a_hi;
    }
    ...
    __forceinline long long &operator[](size_t index)
    {
      assert(index < 4);
      return i[index];
    }
    __forceinline const long long &operator[](size_t index) const
    {
      assert(index < 4);
      return i[index];
    }
  ...
  __forceinline vllong4 vreduce_and(const vllong4 &y)
  {
    const vllong4 x = vreduce_and2(y);
    return x & shuffle2<1, 0>(x);
  }

  __forceinline vllong4 vreduce_or2(const vllong4 &x) { return x | shuffle<1, 0>(x); }
  __forceinline vllong4 vreduce_or(const vllong4 &y)
  {
    const vllong4 x = vreduce_or2(y);
    return x | shuffle2<1, 0>(x);
  }
