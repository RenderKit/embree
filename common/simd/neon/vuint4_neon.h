...
namespace embree
{
  ...
  struct vuint<4>
  {
    ...
    __forceinline vuint(StepTy)   { unsigned int lanes[4] = {0,1,2,3}; v = vreinterpretq_s32_u32(vld1q_u32(lanes)); }
  ...
  template<> __forceinline unsigned int extract<0>(const vuint4& b) { return vgetq_lane_u32(vreinterpretq_u32_s32(b.v), 0); }
