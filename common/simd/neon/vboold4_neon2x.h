...
namespace embree
{
  ...
  struct vboold<4>
  {
    ...
    __forceinline vboold(const float64x2_t &a_lo, const float64x2_t &a_hi)
    {
      v.vl = a_lo;
      v.vh = a_hi;
    }
    ...
    __forceinline vboold(FalseTy)
    {
      v.vl = vdupq_n_f64(0.0);
      v.vh = vdupq_n_f64(0.0);
    }
    ...
    {
      assert(index < 4);
      return ((neon_movemask_pd(v.vl) | (neon_movemask_pd(v.vh) << 2)) >> index) & 1;
    }
