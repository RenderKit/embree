...
{
  if (interval.upper-interval.lower >= float(M_PI)) { return Interval1f(-1.0f, 1.0f); }
  ...
  if (interval.lower <       float(M_PI) / 2.0f && interval.upper >       float(M_PI) / 2.0f) sinUpper =  1.0f;
  if (interval.lower < 3.0f * float(M_PI) / 2.0f && interval.upper > 3.0f * float(M_PI) / 2.0f) sinLower = -1.0f;
...
inline Interval1f cos(Interval1f interval)
{
  ...
  if (interval.lower < float(M_PI) && interval.upper > float(M_PI)) cosLower = -1.0f;
