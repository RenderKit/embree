...
namespace embree
{
  ...
  __forceinline float abs  ( const float x ) { return cl::sycl::fabs(x); }
  __forceinline float acos ( const float x ) { return cl::sycl::acos(x); }
  __forceinline float asin ( const float x ) { return cl::sycl::asin(x); }
  __forceinline float atan ( const float x ) { return cl::sycl::atan(x); }
  __forceinline float atan2( const float y, const float x ) { return cl::sycl::atan2(y, x); }
  __forceinline float cos  ( const float x ) { return cl::sycl::cos(x); }
  __forceinline float cosh ( const float x ) { return cl::sycl::cosh(x); }
  __forceinline float exp  ( const float x ) { return cl::sycl::exp(x); }
  __forceinline float fmod ( const float x, const float y ) { return cl::sycl::fmod(x, y); }
  __forceinline float log  ( const float x ) { return cl::sycl::log(x); }
  __forceinline float log10( const float x ) { return cl::sycl::log10(x); }
  __forceinline float pow  ( const float x, const float y ) { return cl::sycl::pow(x, y); }
  __forceinline float sin  ( const float x ) { return cl::sycl::sin(x); }
  __forceinline float sinh ( const float x ) { return cl::sycl::sinh(x); }
  __forceinline float sqrt ( const float x ) { return cl::sycl::sqrt(x); }
  __forceinline float tan  ( const float x ) { return cl::sycl::tan(x); }
  __forceinline float tanh ( const float x ) { return cl::sycl::tanh(x); }
  __forceinline float floor( const float x ) { return cl::sycl::floor(x); }
  __forceinline float ceil ( const float x ) { return cl::sycl::ceil(x); }
...
*/
  ...
  __forceinline      int min(int      a, int      b) { return cl::sycl::min(a,b); }
  __forceinline unsigned min(unsigned a, unsigned b) { return cl::sycl::min(a,b); }
  __forceinline  int64_t min(int64_t  a, int64_t  b) { return cl::sycl::min(a,b); }
  __forceinline    float min(float    a, float    b) { return cl::sycl::fmin(a,b); }
  __forceinline   double min(double   a, double   b) { return cl::sycl::fmin(a,b); }
#if defined(__X86_64__)
  __forceinline   size_t min(size_t   a, size_t   b) { return cl::sycl::min(a,b); }
#endif
...
//  template<typename T> __forceinline T mini(const T& a, const T& b, const T& c, const T& d, const T& e) { return mini(mini(mini(a,b),mini(c,d)),e); }
  ...
  __forceinline      int max(int      a, int      b) { return cl::sycl::max(a,b); }
  __forceinline unsigned max(unsigned a, unsigned b) { return cl::sycl::max(a,b); }
  __forceinline  int64_t max(int64_t  a, int64_t  b) { return cl::sycl::max(a,b); }
  __forceinline    float max(float    a, float    b) { return cl::sycl::fmax(a,b); }
  __forceinline   double max(double   a, double   b) { return cl::sycl::fmax(a,b); }
#if defined(__X86_64__)
  __forceinline   size_t max(size_t   a, size_t   b) { return cl::sycl::max(a,b); }
#endif
...
//  template<typename T> __forceinline T maxi(const T& a, const T& b, const T& c, const T& d, const T& e) { return maxi(maxi(maxi(a,b),maxi(c,d)),e); }
  ...
  __forceinline float madd  ( const float a, const float b, const float c) { return +cl::sycl::fma(+a,b,+c); }
  __forceinline float msub  ( const float a, const float b, const float c) { return +cl::sycl::fma(+a,b,-c); }
  __forceinline float nmadd ( const float a, const float b, const float c) { return +cl::sycl::fma(-a,b,+c); }
  __forceinline float nmsub ( const float a, const float b, const float c) { return -cl::sycl::fma(+a,b,+c); }
